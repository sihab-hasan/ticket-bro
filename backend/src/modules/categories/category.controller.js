...
class CategoryController {
  ...
  getSubcategories = asyncHandler(async (req, res) => { sendSuccess(res, 'Category subcategories fetched.', await categoryService.getSubcategories(req.params.slug)); });
  create    = asyncHandler(async (req, res) => { sendCreated(res, 'Category created.', { category: await categoryService.create(req.body) }); });
  update    = asyncHandler(async (req, res) => { sendSuccess(res, 'Category updated.', { category: await categoryService.update(req.params.slug, req.body) }); });
  remove    = asyncHandler(async (req, res) => { sendSuccess(res, 'Deleted.', await categoryService.remove(req.params.slug)); });
}
