...
const MOCK_NOTIFICATIONS = [
  {
    ...
    message: 'Your tickets for Taylor Swift: The Eras Tour have been confirmed.',
  ...
  },
  {
    ...
    actions: [
      { label: 'View Receipt', url: '/payments/details/pay_123456' }
    ]
  },
  {
    id: 'notif_3',
...
export const NotificationProvider = ({ children }) => {
  ...
  const initializeWebSocket = () => {
    ...
    // wsRef.current = new WebSocket(`wss://api.ticketbro.com/ws/notifications?userId=${user?.id}`);
  ...
  // Update preferences
  const updatePreferences = useCallback(async (type, channel, enabled) => {
    setPreferences(prev => ({
      ...prev,
      [type]: {
        ...prev[type],
  ...
  const filteredNotifications = useCallback(() => {
    return notifications.filter(notif => {
      ...
      if (filters.isRead !== null && notif.isRead !== filters.isRead) return false;
      if (filters.dateFrom && new Date(notif.createdAt) < new Date(filters.dateFrom)) return false;
      if (filters.dateTo && new Date(notif.createdAt) > new Date(filters.dateTo)) return false;
  ...
  const getUnreadByType = useCallback((type) => {
    return notifications.filter(n => n.type === type && !n.isRead && !n.isArchived);
  }, [notifications]);
  ...
  const cleanupExpired = useCallback(() => {
    const now = new Date();
    setNotifications(prev => 
      prev.filter(n => new Date(n.expiresAt) > now)
    );
  }, []);

  // Schedule notification
  const scheduleNotification = useCallback(async (notification, scheduleTime) => {
    try {
  ...
  const sendTestNotification = useCallback(async (channel = NotificationChannel.IN_APP) => {
